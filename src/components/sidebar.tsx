...
function useSidebarActions(): SidebarActionsContextProps {
    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        ...
        >
          ...
          <nav aria-label="Main navigation" className="flex h-full w-full flex-col">
            {children}
          </nav>
        </SheetContent>
      </Sheet>
...
function SidebarRail({ className, ...props }: React.ComponentProps<typeof IconButton>) {
  const { toggleSidebar } = useSidebarActions();
...
function SidebarMenuButton({
  ...
  // Tooltips only useful when sidebar is collapsed to icon mode
  if (tooltip === undefined || state !== 'collapsed' || isMobile) {
    return button;
  }
